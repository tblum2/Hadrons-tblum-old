...
// MCA - need to change this for nucleons?
...
public:
	...
	static inline void contNucTen(Mat &spinMat, const TenLeft &a, const TenRight &b)
	{
		...
		//parallel_for (int mu = 0; mu < Ns; mu++)
        thread_for(mu, Ns,{
			//parallel_for (int nu = 0; nu < Ns; nu++)
			thread_for(nu,Ns,{
...
{
    return Ns*nt_*ni_*nj_*nk_*sizeof(T);
}
...
{
#ifdef HAVE_HDF5
    std::vector<hsize_t>    dim = {static_cast<hsize_t>(Ns),
                               ...
                               const unsigned int blockSizek)
{
#ifdef HAVE_HDF5
    Hdf5Reader           reader(filename_, false);
    std::vector<hsize_t> count = {Ns, nt_, blockSizei, blockSizej, blockSizek},
                         offset = {static_cast<hsize_t>(str), 0, static_cast<hsize_t>(i),
    ...
    push(reader, dataname_);
    auto &group = reader.getGroup();
    dataset     = group.openDataSet(HADRONS_A2AN_NAME); // MCA - change this?
...
void A2AMatrixNucIo<T>::load(Vec<VecT> &v, double *tRead)
{
    ...
    push(reader, dataname_);
    auto &group = reader.getGroup();
    dataset     = group.openDataSet(HADRONS_A2AN_NAME); // MCA - need to change this?
	...
	// MCA - need to adjust this section from matrix to rank 3 tensor
    A2AMatrixNuc<T>         buf(Ns, ni_, nj_, nk_); // MCA - need to change this into a tensor
    std::vector<hsize_t> count    = {static_cast<hsize_t>(Ns), 1,
									 static_cast<hsize_t>(ni_),
                                     static_cast<hsize_t>(nj_),
                                     static_cast<hsize_t>(nk_)},
                         stride   = {1, 1, 1, 1, 1},
                         ...
                         memCount = {static_cast<hsize_t>(Ns),
                                     ...
                                     static_cast<hsize_t>(nj_),
                                     static_cast<hsize_t>(nk_)};
    /*std::vector<hsize_t> count    = {static_cast<hsize_t>(1), 1, static_cast<hsize_t>(10), static_cast<hsize_t>(10), static_cast<hsize_t>(10)},
                         stride   = {static_cast<hsize_t>(4), 1, static_cast<hsize_t>(20), static_cast<hsize_t>(20), static_cast<hsize_t>(20)},
                         block    = {static_cast<hsize_t>(4), 1, static_cast<hsize_t>(20), static_cast<hsize_t>(20), static_cast<hsize_t>(20)},
                         memCount = {static_cast<hsize_t>(Ns),
									 static_cast<hsize_t>(ni_),
                                     static_cast<hsize_t>(nj_),
                                     static_cast<hsize_t>(nk_)};*/
    ...
    *tRead = 0.;
    for (unsigned int tp1 = nt_; tp1 > 0; --tp1)
    {
        ...
        std::vector<hsize_t> offset = {0, static_cast<hsize_t>(t), 0, 0, 0};
          ...
          const FilenameFn &filenameFn, const MetadataFn &metadataFn)
{
    ...
    for(int k=0;k<N_k;k+=blockSize_)
    {
        ...
        A2AMatrixSetNuc<TIo> mBlock(mBuf_.data(), next_, nstr_, nt_, N_ii, N_jj, N_kk);
        ...
        for(int s =0;s< nstr_;s++)
        for(int t =0;t< nt_;t++)
        for(int a=0;a< N_ii;a++)
        for(int b=0;b< N_jj;b++)
        for(int c=0;c< N_kk;c++)
        {
			
            if (t == 0)
            {
				LOG(Message) << t << " " << s << " " << a << " " << b << " " << c << " "
					<< mBlock(e,s,t,a,b,c) << std::endl;
...
#endif
        STOP_TIMER("IO: total");
        blockSize  = static_cast<double>(next_*nstr_*nt_*N_ii*N_jj*N_kk*sizeof(TIo));
        ioTime    += GET_TIMER("IO: write block");
